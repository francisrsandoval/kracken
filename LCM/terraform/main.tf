/*
  This template demonstrates:
    ...
    * turn on SR-IOV (accelerated-networking in Az)
    * Add 2 network interfaces to the VM (mgmt, data)
    * execute a script on the VM
*/
...
resource "azurerm_public_ip" "OpenSpacePIP-data" {
    name                         = "OpenSpacePIP-data"
    resource_group_name          = azurerm_resource_group.OpenSpaceRG.name
    location                     = azurerm_resource_group.OpenSpaceRG.location
    allocation_method            = "Dynamic"
...
resource "azurerm_network_interface" "dataNIC" {
    ...
    ip_configuration {
        ...
        subnet_id                     = azurerm_subnet.OpenSpaceSubnet.id
        private_ip_address_allocation = "Dynamic"
        public_ip_address_id          = azurerm_public_ip.OpenSpacePIP-data.id
    }
...
# Connect the security group to the network interface
resource "azurerm_network_interface_security_group_association" "dataAssociation" {
...
resource "azurerm_linux_virtual_machine" "OpenSpaceVM" {
    ...
    resource_group_name   = azurerm_resource_group.OpenSpaceRG.name
    location              = azurerm_resource_group.OpenSpaceRG.location
    network_interface_ids = [azurerm_network_interface.dataNIC.id, azurerm_network_interface.mgmtNIC.id]
...
*/
    # reference our image built w packer
